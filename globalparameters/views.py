...
def validate_login_request(request, request_url):
    try: 
        ...
        if response.status_code == 200:
            ...
            # persist useful session values if present
            request.session['username'] = resp_json.get('username')
            request.session['isAdmin'] = resp_json.get('isAdmin')
            request.session['temp_session_id'] = resp_json.get('temp_session_id')

            return JsonResponse(success_message, status=200)
        else:
            # return API error body if possible
            try:
                body = response.json()
            except Exception:
                body = response.text
            return JsonResponse({'resultCode': '-100', 'resultDescription': body}, status=response.status_code)
   
    except Exception as e:
         return JsonResponse({'resultCode': '-104', 'resultDescription': str(e)}, status=500)
